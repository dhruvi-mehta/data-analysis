...
This project investigates a deceptively simple question:
...
- **Regional disparities are stark.** States in the Southeast and parts of the Midwest showed alarmingly low facility density relative to population, while states in the Northeast had higher density. Yet facility density did not translate directly into better outcomes.

**Bottom line:** Access to care is not the same as ability to use it. Structural conditions including income, education, and insurance coverage shape mental health outcomes more than physical proximity to facilities.
...
- **Unit:** Individual survey responses, aggregated to state level
- **File:** `LLCP2023.XPT`
- **Download:** [https://www.cdc.gov/brfss/annual_data/annual_2023.html](https://www.cdc.gov/brfss/annual_data/annual_2023.html)
...
- **Unit:** Facility-level data, aggregated to state level
- **File:** `NSUMHSS_2023_PUF_CSV.csv`
- **Download:** [https://www.samhsa.gov/data/](https://www.samhsa.gov/data/)
...
- Aggregated BRFSS to state level
...
1. **Choropleth Map** - Mental health facility density per 100k people by state
2. **Scatter Plot** - Facility density vs. average poor mental health days
3. **Scatter Plot** - Facility density vs. depression rate
4. **Scatter Plot** - SES index (PCA) vs. facility density
...
```r
install.packages(c("haven", "tidyverse", "maps", "viridis"))
```
...
2. Download the raw data files (too large for GitHub) and place them in the `data/` folder:
   - **BRFSS 2023:** Download `LLCP2023.XPT` from the [CDC BRFSS page](https://www.cdc.gov/brfss/annual_data/annual_2023.html)
   - **NSUMHSS 2023:** Download `NSUMHSS_2023_PUF_CSV.csv` from [SAMHSA](https://www.samhsa.gov/data/)

3. Open `analysis.R` and run the full script. It is recommended to use RStudio with the included `.Rproj` file, which sets the working directory automatically.
...
*Analysis conducted using R. Data from 2023 federal public-use datasets.*
